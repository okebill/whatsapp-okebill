-- ===================================
-- EGLOBALTECH WA-GATEWAY MIGRATION
-- Multi-Tenant SaaS Platform
-- ===================================

-- Update users table for multi-tenant and billing
ALTER TABLE users ADD COLUMN IF NOT EXISTS whatsapp_number VARCHAR(20) UNIQUE;
...
ALTER TABLE users ADD COLUMN IF NOT EXISTS subscription_type ENUM('free', 'monthly', 'yearly') DEFAULT 'free';
ALTER TABLE users ADD COLUMN IF NOT EXISTS expired_date DATE;
...
ALTER TABLE users ADD COLUMN IF NOT EXISTS payment_status ENUM('active', 'expired', 'suspended', 'trial') DEFAULT 'trial';
ALTER TABLE users ADD COLUMN IF NOT EXISTS trial_ends_at DATE;
...
-- Update message_stats for multi-tenant
ALTER TABLE message_stats ADD COLUMN IF NOT EXISTS connection_id VARCHAR(50);
ALTER TABLE message_stats ADD COLUMN IF NOT EXISTS target_type ENUM('individual', 'group') DEFAULT 'individual';
...
INSERT INTO users (username, password, role, is_active, payment_status, expired_date, approved_at, approved_by)
SELECT 'admin', '$2a$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'admin', 1, 'active', '2030-12-31', NOW(), 1
